  ...
  <div class="dossier-info">
    ...
    <v-dialog v-model="dorkDialog" max-width="560">
      <div class="glass-card dialog-card">
        ...
        <div class="di-dork-list">
          <div v-for="(dork, i) in dorkQueries" :key="i" class="di-dork-item">
            <div class="di-dork-label">{{ dork.label }}</div>
            <div class="di-dork-links">
              <a v-for="link in dork.links" :key="link.engine" :href="link.url" target="_blank" rel="noopener" class="di-dork-link" :title="link.engine">
                <img v-if="link.engine === 'Google'" src="https://www.google.com/favicon.ico" width="14" height="14" alt="G" />
                <img v-else-if="link.engine === 'Bing'" src="https://www.bing.com/favicon.ico" width="14" height="14" alt="B" />
                <img v-else-if="link.engine === 'Yandex'" src="https://yandex.com/favicon.ico" width="14" height="14" alt="Y" />
                <img v-else-if="link.engine === 'DuckDuckGo'" src="https://duckduckgo.com/favicon.ico" width="14" height="14" alt="D" />
...
// ─── Google Dorking ────────────────────────────────────────────────────────────
interface DorkLink { engine: string; url: string }
interface DorkQuery { label: string; query: string; links: DorkLink[] }

const dorkDialog = ref(false);
const dorkEntityName = ref('');
const dorkQueries = ref<DorkQuery[]>([]);

function buildSearchLinks(query: string): DorkLink[] {
...
function generateDorkQueries(name: string, type: string): DorkQuery[] {
  const queries: DorkQuery[] = [];
  const q = (query: string, label: string) =>
    queries.push({ label, query, links: buildSearchLinks(query) });

  // Universal: exact match
  ...
  if (type === 'email') {
    ...
    q(`"${name}" entreprise OR société OR company`, t('dossier.dorkCompany'));
    ...
    q(`"${name}" entreprise OR société`, t('dossier.dorkCompany'));
  } else {
    // Social media types: facebook, instagram, twitter, tiktok, etc.
    q(`"${name}" site:google.com`, t('dossier.dorkGeneral'));
    q(`"${name}" site:facebook.com`, 'Facebook');
    q(`"${name}" site:instagram.com`, 'Instagram');
    q(`"${name}" site:linkedin.com`, 'LinkedIn');
    q(`"${name}" filetype:pdf`, t('dossier.dorkPDF'));
...
function openDorkDialog(index: number) {
  const entity = form.entities[index];
  if (!entity) return;
  dorkEntityName.value = `${entityTypeLabel(entity.type)} : ${entity.name}`;
  dorkQueries.value = generateDorkQueries(entity.name, entity.type);
  dorkDialog.value = true;
}

function copyAllDorks() {
  const text = dorkQueries.value.map(d => `${d.label}\n${d.query}\n${d.links[0]?.url || ''}`).join('\n\n');
  navigator.clipboard.writeText(text);
}

async function sendDorksToNote() {
  if (!dossierStore.currentDossier) return;
  const content: any[] = [
    {
      type: 'heading',
      attrs: { level: 2 },
      content: [{ type: 'text', text: `Google Dorking — ${dorkEntityName.value}` }],
    },
  ];

  for (const dork of dorkQueries.value) {
    content.push({
      type: 'heading',
      attrs: { level: 3 },
      content: [{ type: 'text', text: dork.label }],
    });
    content.push({
      type: 'paragraph',
      content: [
  ...
  try {
    const { data } = await api.post('/nodes', {
      ...
      parentId: null,
      type: 'note',
      title: `Google Dorking — ${dorkEntityName.value}`,
      content: { type: 'doc', content },
    });
    dossierStore.nodes.push(data);
    dossierStore.selectNode(data);
    dorkDialog.value = false;
  } catch (err) {
